...
FEATURE_TOGGLES: tuple[FeatureToggleDefinition, ...] = (
    ...
    FeatureToggleDefinition(
        ...
        description=(
            ...
            "стратегия dnat_front (рекомендуется) переключает на стороне сервера через proxy_agent "
            "и не требует ничего на устройстве; client_sync — старый способ с несколькими профилями "
            "и ручным/устройственным переключением."
        ),
        icon="🔀",
    ...
    FeatureToggleDefinition(
        ...
        description=(
            "Пересборка фронтенда и связанные действия — вкладка внутри Настроек "
            "(Настройки → Операции панели), не отдельный пункт в боковом меню. "
            "Не связано с автопереключением узлов (см. «Автопереключение (пулы серверов)»)."
        ),
        icon="🛠️",
