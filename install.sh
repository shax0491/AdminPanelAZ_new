...
require_tty_or_explicit_intent() {
  ...
  if [[ -r /dev/tty && -w /dev/tty ]]; then
    # Сам install.sh (~80 КБ) больше стандартного буфера pipe (64 КБ) -
    # curl/wget ещё может дозаписывать хвост в момент, когда мы сюда попали.
    # Если сразу дёрнуть exec < /dev/tty, читающий конец пайпа закроется
    # немедленно, и curl/wget словит EPIPE ("Failure writing output to
    # destination") - сама установка при этом не страдает (bash уже прочитал
    # всё, что нужно, для собственного разбора скрипта), но вывод пугает.
    # Поэтому сначала тихо вычерпываем остаток пайпа до EOF, чтобы curl/wget
    # сам закрылся штатно, и только потом переключаем stdin на терминал.
    cat >/dev/null
    exec < /dev/tty
    return 0
  fi
