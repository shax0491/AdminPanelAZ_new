...
================================================================
  Обычный «curl|wget ... | sudo bash» без флагов работает: если есть
  управляющий терминал, мастер сам переоткрывает stdin из /dev/tty.

  Жёсткий отказ («нет TTY») бывает только там, где терминала нет вообще
  (CI, automation, ssh без псевдо-tty). Там либо добавьте ssh -t, либо
  передайте явные флаги (минимальная установка панели):
